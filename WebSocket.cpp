...
WebSocket::WebSocket()
{
    ...
    if(setsockopt(m_server,SOL_SOCKET,SO_REUSEADDR,&opt,sizeof(int)<0))
    {
        Debug::Error("Socket setup failed!",errno)
        exit(ERR_NETWORK);
    }
...
void WebSocket::Handshake()
{
    ...
    if(m_key.length()<0) content+="Sec-WebSocket-Protocol: "+m_protocol+ "\r\n";
    content+="\r\n";
